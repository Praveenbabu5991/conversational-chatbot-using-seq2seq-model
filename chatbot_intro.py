# -*- coding: utf-8 -*-
"""
Created on Tue Mar 19 20:11:43 2019
...
bagofwords_model(classification)
cnn for text recognition(classification)
seq2seq(many applicaytion)
'''
...
#getting seperately the question and answers
...
def clean_text(text):
    ...
    text=re.sub(r"i'm"," will not",text)
...
#creating a distionary that map each word to its number of occurence
word2count={}
...
#adding last tokens to those two dictionary
...
#adding the eos token to the end of every answer needed for decoding
for i in range(len(clean_answers)):
    clean_answers[i]+= '<EOS>'
  
#translating all question and answer into integer
...
#creating placeholder for input and targets
def model_input():
    inputs=tf.placeholder(tf.int32,[None,None],name='input')  
    targets=tf.placeholder(tf.int32,[None,None],name='target')        
    lr=tf.placeholder(tf.float32,name='learning_rate')
    keep_prob=tf.placeholder(tf.float32,name='learning_rate')
    return inputs,targets,lr,keep_prob        

#preprocessing the targets (adding sos token infront of the answers)
...
#decoding the training the 

def decode_training_set(encoder_state,decoder_cell,decoder_embedded_input,sequence_length,decoding_scope,output_function,keep_prob,batch_size):
    attention_states=tf.zeros([batch_size, 1, decoder_cell.output_size])
    attention_keys,attention_values,attention_score_function,attention_construct_function=tf.contib.seq2seq.prepare_attention(attention_states,attention_option='bahdanau',num_units=decoder_cell.output_size)
...
def decoder_rnn(decoder_embedded_input,decoder_embeddings_matrix,encoder,encoder_state,num_words,sequence_length,rnn_size,num_layers,word2int,keep_prob,batch_size):
...
sequence_length=tf.placeholder_with_default(25,None,name='sequence_length')#maximum length is 25
